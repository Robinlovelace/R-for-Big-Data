...
A common definition for Big Data is that which is:
...
Precisely how 'V' data needs to be before it's classified as 'Big' is rarely specified, however.
A looser definition from the literature recognises that Big Data an umbrella or 'catch all' term: in practice, and especially in the social sciences, Big Data is used simply to refer to information that is "difficult  to  analyze using established methods" [@Lovelace2015].

The variety of new datasets becoming available
is such that no single tutorial can
cover all shapes and sizes. Gaining a solid
understanding of the underlying R *language*
and clearly defining what you want to do is therefore more
important than knowing the 'right' function or
package. In most cases there are many ways to solve
a problem with R. The 'best' one depends on your
...
for DIY (Fig. 1). People rarely ask 'is this the BEST possible drill'?.
More likely a good builder will ask: 'Is this drill *good enough* to get the job done?' More importantly 'Is this drill robust?' and 'Will it work in 20 years time?' The same applies to R for Big Data.
