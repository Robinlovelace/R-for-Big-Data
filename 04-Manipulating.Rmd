...
To clean the giant dataset, removing the empty colums, we need to identify which these variables are.
...
> **Challenge:** rename the variables 3 to 7 so they are much shorter, followin the pattern `ECO2`, `MCO2` etc. That will make the code for manipulating the dataset easier to write

```{r, echo=FALSE}
names(df)[3:7] <- c("ECO2", "MCO2", "TCO2", "OCO2", "FCO2")
```
...
An easier way to aggregate data is using the tilde (`~`) sybol instead of specifying grouping variable as a list...
