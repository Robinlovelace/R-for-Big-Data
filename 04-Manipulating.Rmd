...
Two common operations needed to tidy are datasets reshaping and splitting. Each of these has its own **tidyr** function, as described
below:
...
The latter may not be as easy to visualise, but is more appropriate for efficient analysis in R.
Note also that when we print the object `rawt[1:3,]`, the class of each variable is given
(`chr`, `fctr`, `int` refer to character, factor and integer classes, respectively).
This is because `read_csv` uses the `tbl` class from **dplyr**, which extends data.frame.
]
...
\end{tabular}
\vspace{0.2cm}
\caption{Dataframe after the 'agesex' variable has been split into age and sex.}\label{Tsep}
\end{margintable}
...
fast and intuitive.
**dplyr** works best with tidy data, as described above with reference to **tidyr**.
...
```{r, message=FALSE}
library(readr)
idata <- read_csv("data/world-bank-ineq.csv")
library(dplyr)
...
```{r}
aus_idata1 <- idata[idata$Country == "Australia",]
```
...
\noindent As we will see with Twitter data in a subsequent section,
