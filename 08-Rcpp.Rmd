...
In this chapter, C and C++ code are largely interchangeable, so when you see 'C code', it can usually be included in a `.cpp` file\sidenote{`.cpp` is the default file extension for C++ scripts.}. In general, this isn't true. See for example the Stackoverflow question, \url{http://programmers.stackexchange.com/q/16390/14846} for an overview.
...
To use\sidenote{To use, I mean write functions. If you distribute code in a package, then this isn't an issue. For example, the `ggplot2` package uses Rcpp.} the package you need a working C++ compiler. 
...
\noindent The associated CRAN\sidenote{\url{https://cran.r-project.org/web/packages/Rcpp/}} page has numerous vignettes that are worth looking at\sidenote{You can get an idea of the popularity of the package by looking at the `Reverse Imports` section.}
...
4. You don't use assignment when creating a function.
...
\noindent In C++, this is a bit more long winded
...
\noindent The beauty of using Rcpp is that it make it very easy to call C++ functions from R.
...
\noindent Then we simply pass the C++ function as an argument
...
\noindent When we can call in the usual way

```{r}
add_c(1, 2)
```

\noindent Notice that if you included this function in a package, users don't have to worry about any of the Rcpp magic. It just works.
...
The `cppFunction` is great for getting small examples up and running. But it is better practice to put your C++ code in a separate file (with file extension `cpp`) and use the function call `sourceCpp("path/to/file.cpp")` to compile them. However, we need to include a few headers at the top of the file. The first line we add gives us access to the Rcpp functions. The file `Rcpp.h` contains a list of function and class definitions supplied by Rcpp. The `include` statement, adds the definitions to the top of your code
...
In this example, we'll let Rcpp smooth the interface between C++ and R by using the `NumericVector` data type. This Rcpp data type mirrors the R vector object type. Other common classes are: `IntegerVector`, `CharacterVector`, and `LogicalVector`. 
