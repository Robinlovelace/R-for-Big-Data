...
Note that C and C++ code is largely interchangeable, so when you see 'C code', it can usually be included in a `.cpp` file.^[`.cpp` is the default file extension for C++ scripts.

**Mini challenge**: guess what cpp means!]
...
\bottomrule
\end{tabular}
\caption{Some basic object types in C.}
\end{table}
