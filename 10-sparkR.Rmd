...
count(mobys)
```
\noindent Spark deals with Transformations and actions in two different ways. Similar to `dplyr`, Spark uses lazy evaluation, that is it only performs that computation when it is used by an action. In the example above, the `textFile` and `filterRDD` commands are run only when we use `count`. 
