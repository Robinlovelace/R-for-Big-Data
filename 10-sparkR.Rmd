...
The Spark stack is composed of the following libraries.
  ...
  * Third party libraries: Like R and Python, developers are encouraged to
  ...
  each of which can be rated by the community (see
  [spark-packages.org](http://spark-packages.org/)).


# A first Spark instance

There are a number of preliminary steps we need to take before beginning an analysis. After Spark had been installed^[There are a number of ways to deploy Spark, the simplest of which is 'Standalone Mode', which simply requires a compiled version of Spark to be available on the machine. Other deployment modes include 'EC2', for use on Amazon's cloud computing infrastructure, and via Apache's cluster management system Mesos and YARN, which is an evolution of Hadoop.] we set the environment variable `SPARK_HOME`. This can be in done in our `bashrc` file, or in R itself, via
...
Spark's RDDs are (by default) recomputed each time you run an action on them.\sidenote{Alternatively, we could use `cache(moby)`, which is the same as `persist` with the default level of storage.} To reuse RDDs in multiple operations we can ask Spark to persist it via
```{r eval=FALSE}
...
There are two ways of creating an RDD: 1) by parallelizing an existing dataset; or 2) from an external data source, such as a database or csv file.
