...
Suppose we have already initialised a Spark context. To use Spark dataframes, we need to create an `SQLContext`, via
```{r eval=FALSE}
sqlContext = sparkRSQL.init(sc)
```

\noindent The SQLContext enables us to create dataframes from a local R data frame, 
