...
There are a number of preliminary steps we need to take before beginning an analysis. First, we set the environment variable `SPARK_HOME`. This can be in done in our `bashrc` file, or in R itself, via
...
Then we load the `SparkR` packag
```{r eval=FALSE}
library("SparkR")
```
...
table'.\sidenote{A hive is a data structure used by Haddoop.} 
