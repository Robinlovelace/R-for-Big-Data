...
\end{quote}
So before we rush headlong into optimising our R code, we will spend some time determining when it is worthwhile optimising. In this chapter, we will look at benchmarking. In computing, a benchmark is obtained by running a set of programs in order to assess their relative performance.
...
\begin{itemize}
\item \texttt{user} is the CPU time (in secs) charged for the execution of user
  instructions of the calling process.
\item \texttt{system} is the CPU time (in secs) charged for execution by the
  system on behalf of the calling process.
\item \texttt{elapsed} is the real elapsed time (in secs) since the process was
  started.
\end{itemize}
...
To compare over different values of $n$, we just loop:\sidenote{This piece of code is breaking the number one rule in efficient R programming - see the next chapter for details.}
```{r cache=TRUE, tidy=FALSE}
d = NULL
for(n in 10^(5:7)) {
...
\begin{enumerate}
\item Put any object creation outside the loop. For example
```{r cache=TRUE, tidy=FALSE}
...
for(i in 1:length(parts))
    post[i] = jitter(parts[i], k)
```
\noindent or even better, just
```{r cache=TRUE}
post = sapply(parts, jitter, k)
```
\item Make the number of iterations as small possible. For example, if you have the choice
