...
summary(edu_f) # note the order these are prined
```
...
the internet and guess how the plots in Fig. 3 were created. (Recoding the data,
as illustrated in the next section, will help.)
...
edu_f <- factor(edu_f, levels(edu_f)[c(2, 1, 4, 3)]) # change levels order
summary(edu_f)
```

```{r, echo=FALSE, fig.cap="Different ways of plotting level of education of participants in the WCS"}
ord <- c(2, 1, 4, 3, 5)
...
n <- is.na(edu_f) # edu_f values that are not

bp <- 
  ggplot() +
  geom_bar(aes(x = "", fill = edu_f), stat = "bin",
    width = 1)
